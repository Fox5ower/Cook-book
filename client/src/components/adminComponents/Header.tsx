...
class Header extends Component<MyProps, MyState> {
  ...
  UNSAFE_componentWillMount() {
    tokenInterceptor()
    axios.get(`${DEV_URL}/api/admin/${this.props.id}`).then(admin => {
        ...
        <div className="pannel__header">
          ...
          <div className="header__admin-actions">
            <div className="miniature">
              {this.state.admin.name.toUpperCase()[0]}
              <div className="actions">
                <div className="info">
                  {this.state.admin.name.toUpperCase()}
                  <hr />
                </div>
                <div
                  className="action"
                  onClick={() => this.openModal('logout')}
                >
                  <FormattedMessage
                ...
                >
                  <FormattedMessage
                    id="admin.modal.change.pass"
                    defaultMessage="Change Password"
                  />
                </div>
                <div
                  className="action"
                  onClick={() => this.openModal('information')}
                >
                  <FormattedMessage
                    id="admin.modal.change.info"
                    defaultMessage="Change Info"
                  />
                </div>
              </div>
            </div>
