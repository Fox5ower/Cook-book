...
class Modal extends Component<MyProps, MyState> {
  ...
  submitHandler = (e: SyntheticEvent) => {
    ...
    if (
      ...
      axios
        .put(`${DEV_URL}/api/admin/${this.props.actionType}`, this.state)
        .then(res => {
          if (res.status === 200) {
            this.setState({
              password: '',
          ...
          <div className="modal">
            <form id="form" method="POST" action="/api/admin/password">
              <fieldset className="column-fieldset">
                <legend>
                  <FormattedMessage
                    id="admin.modal.changePass.header"
                    defaultMessage="Change Password"
                  />
                </legend>
                <FormError></FormError>
                <FormattedMessage
                  id="admin.modal.changePass.pass.placeholder"
                ...
                <FormattedMessage
                  id="admin.modal.changePass.confirmPass.placeholder"
          ...
          <div className="modal">
            <form id="form" method="POST" action="/api/admin/password">
              <fieldset className="column-fieldset">
                <legend>
                  <FormattedMessage
                    id="admin.modal.changeInfo.header"
                    defaultMessage="Change Information"
                  />
                </legend>
                <FormError></FormError>
                <FormattedMessage
                  id="admin.modal.changeInfo.email.placeholder"
                ...
                <FormattedMessage
                  id="admin.modal.changeInfo.name.placeholder"
                  defaultMessage="Name"
                >
                  {(placeholder: string) => (
                    <Input
