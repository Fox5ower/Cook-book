...
class AddPage extends Component<MyProps, MyState> {
  ...
  submitHandler = (e: any) => {
    ...
    axios.post(`${DEV_URL}/api/panel/add`, body_en).then(() => {
      axios.post(`${DEV_URL}/api/panel/add`, body_ru).then(res => {
        if (res.status === 200) {
          this.setState({
            redirect: true,
          })
      ...
      <div className="dish-form__container">
        <div className="dish-form">
          <button
            id="goLeft"
            style={{ width: '7%' }}
            onClick={(e: SyntheticEvent) => this.sliderHandler(e)}
          >
          ...
          <div className="dish-form__header">
            <span>
              <FormattedMessage
                id="admin.dish.header"
                defaultMessage="Add New Dish"
              />
            </span>
          </div>
          <form
            ...
            <fieldset className="en-fieldset">
              ...
              <fieldset className="row-fieldset">
                ...
                <FormattedMessage
                  ...
                  defaultMessage="Engreediants"
                >
                  {(placeholder: string) => (
                    <Input
                      placeholder={placeholder}
                      name="engreediants_en"
                      maxLength={150}
                      onChange={(e: any) => this.changeHandler(e)}
                    ></Input>
                  )}
              ...
              <fieldset className="column-fieldset">
                ...
                <select disabled form="form" name="language_en" id="language">
                  <option disabled hidden selected value="English">
                    English
                  </option>
                </select>
              </fieldset>
